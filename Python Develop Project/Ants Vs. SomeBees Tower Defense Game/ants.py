...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        self.is_damage_doubled = False  # Mark is Ant has double damage
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container == True and ContainerAnt.can_contain(place.ant, self):
                place.ant = place.ant
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if self.is_damage_doubled == False:
            #if isinstance(self, FireAnt):
            #    if self.health <= 0:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current.is_hive == False:
            if current.bees and (count >= self.lower_bound) and (count <= self.upper_bound):
...
class FireAnt(Ant):
    ...
    name = 'Fire'
    damage = 3  # additional amount of damage when it dies
    food_cost = 5
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 5
    implemented = True   # Change to True to view in the GUI
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if self.place.bees:
                current_bee = random_bee(self.place.bees)
                Insect.reduce_health(current_bee, current_bee.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while behind_place:
            if behind_place.ant:
                if behind_place.ant.is_container and behind_place.ant.ant_contained:
...
class LaserAnt(ThrowerAnt):
        ...
        while current_place.is_hive == False:
            if current_place.ant:
                if current_place.ant.is_container:
                    insect_dic[current_place.ant] = place_count
                    if current_place.ant.ant_contained and place_count != 0:
                        insect_dic[current_place.ant.ant_contained] = place_count
                elif current_place.ant.is_container == False and place_count != 0:
                    insect_dic[current_place.ant] = place_count
            
            #print("DEBUG:", current_place.bees)
            if current_place.bees:           
                bees = list(current_place.bees)
                #print("DEBUG:", bees)
                for bee in bees:
                    insect_dic[bee] = place_count
            current_place = current_place.entrance
            place_count = place_count + 1
        #print("DEBUG:", insect_dic)
        return insect_dic
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        ...
        self.damage = self.damage - 0.0625
        #print("DEBUG:", damage)
        return float(damage)
        # END Problem Optional 2
